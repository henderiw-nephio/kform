...
func (r *typeChecker) Check(rootPkg *Package) {
	// use a sub-checker with the appropriate settings
	(&typeChecker{
		NodeFilters:     r.NodeFilters,
		checkedPackages: r.checkedPackages,
	}).check(rootPkg)
}

func (r *typeChecker) isNodeInteresting(node ast.Node) bool {
	// no filters --> everything is important
	if len(r.NodeFilters) == 0 {
		return true
	}

	// otherwise, passing through any one filter means this node is important
	for _, filter := range r.NodeFilters {
		if filter(node) {
			return true
		}
	}
...
func (r *typeChecker) check(rootPkg *Package) {
	rootPkg.Lock()
	defer rootPkg.Unlock()

	r.Lock()
	_, ok := r.checkedPackages[rootPkg]
	r.Unlock()
	if ok {
		return
	}

	refedPackages := allReferencedPackages(rootPkg, r.isNodeInteresting)
	...
	r.Lock()
	defer r.Unlock()
	r.checkedPackages[rootPkg] = struct{}{}
}
