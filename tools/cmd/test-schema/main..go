...
const (
	providerSchema = "config/provider_schema.yaml"
	providerData   = "config/provider_data.yaml"
)

func main() {
	ps, err := os.ReadFile(providerSchema)
	if err != nil {
		panic(err)
	}
	pd, err := os.ReadFile(providerData)
	if err != nil {
		panic(err)
	}
	pData := map[string]any{}
	if err := yaml.Unmarshal(pd, &pData); err != nil {
		panic(err)
	}
	fmt.Println(pData)
	...
	/*
		...
		openapiType, err := loader.LoadFromData(data)
		if err != nil {
			panic(err)
		}
		schema, err := openapiType.Components.Schemas.JSONLookup("io.k8s.provider.v1.Provider")
		if err != nil {
			panic(err)
		}
		fmt.Printf("%#v\n", schema)
	*/
	...
	apiProps := &apiext.JSONSchemaProps{}
	if err := yaml.Unmarshal(ps, apiProps); err != nil {
		panic(err)
	}
	v, _, err := NewSchemaValidator(apiProps)
	if err != nil {
		panic(err)
	}
	//fmt.Println(spec.Properties)
	x := v.Validate(pData)
	fmt.Println(x.IsValid(), x.Errors)
}
